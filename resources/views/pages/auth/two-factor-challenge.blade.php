    ...
    <x-auth::elements.container>
        <div x-data x-on:code-input-complete.window="console.log(event); $dispatch('submitCode', [event.detail.code])" class="relative w-full h-auto">
            ...
            <x-auth::elements.heading
                :text="__('auth.twoFactorChallenge.headline_auth')"
                :description="__('auth.twoFactorChallenge.subheadline_auth')"
                :show_subheadline="($settings->twoFactorChallenge_show_subheadline_auth ?? false)" />
            @else
            <x-auth::elements.heading
                :text="__('auth.twoFactorChallenge.headline_recovery')"
                :description="__('auth.twoFactorChallenge.subheadline_recovery')"
                :show_subheadline="($settings->twoFactorChallenge_show_subheadline_recovery ?? false)" />
            @endif
            ...
            <div class="space-y-5">
                ...
                @enderror
                <x-auth::elements.button rounded="md" submit="true" wire:click="submitCode(document.getElementById('auth-input-code').value)">{{__('Continue')}}</x-auth::elements.button>
                @else
                <div class="relative">
